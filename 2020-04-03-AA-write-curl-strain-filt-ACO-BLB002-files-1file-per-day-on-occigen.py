...
for t in np.arange(len(date_list)):
	...
	strain00_filt_da.to_dataset().to_netcdf(path='eNATL60ACO-BLB002_'+date_list[t]+'_sostrainoverf10m_filt2T.nc',mode='w',engine='scipy')

for t in np.arange(len(date_list)):
	curl00_filt_1day = curl00_filt.squeeze()[24*t:24*(t+1)]
	curl00_filt_da=xr.DataArray(curl00_filt_1day,dims=['time_counter','y','x'],name="curl_Tide_Filt")
	curl00_filt_da.attrs['Name']='eNATL60ACO-BLB002_'+date_list[t]+'_socurloverf10m_filt2T.nc'
	curl00_filt_da.to_dataset().to_netcdf(path='eNATL60ACO-BLB002_'+date_list[t]+'_socurloverf10m_filt2T.nc',mode='w',engine='scipy')
